---
title: "Hello Contract (Truffle + Remote Node)"
sidebarTitle: "Interact with Contracts"
description: "How to deploy and interact your own smart contracts using a remote node."
---

<Tip>
  As Celo is fully EVM compatible, we inherit the rich developer ecosystem and
  tooling of the Ethereum community. You will be deploying a typical hello world
  smart contract onto the Alfajores testnet with the common Ethereum tool,
  Truffle.
</Tip>
...
<Info>
  [Learn more about the Truffle development framework
  here.](https://www.trufflesuite.com/docs/truffle/overview)
</Info>
...
[→Celo Overview/general](/general)

[Clone this Truffle project from GitHub to get started](https://github.com/critesjosh/hello%5Fcontract-truffle).
...
## Prepare Deployment[](#prepare-deployment "Direct link to heading")
...
```java migrations/2_deploy_helloworld.js
...
<Info>
  You can [learn more about Truffle configuration options
  here.](https://www.trufflesuite.com/docs/truffle/reference/configuration)
</Info>

## Deploy to Alfajores (Remotely)[](#deploy-to-alfajores-remotely "Direct link to heading")
...
#### Truffle Deployment[](#truffle-deployment "Direct link to heading")
...
<Info>
  Truffle doesn't estimate the gas properly, so you need to specify a gas limit
...
#### Custom Node.js Deployment[](#custom-nodejs-deployment "Direct link to heading")
...
<Info>
  To deploy a contract on Celo, use the `kit.connection.sendTransaction()`
  function with no `to:` field and the contract bytecode in the `data` field.
  The account that you are sending the transaction from must have enough CELO to
  pay the transaction fee, unless you specify another currency as the
  `feeCurrency`, then you need enough of that currency to pay the transaction
  fee.
</Info>
...
### Interacting with Custom Contracts[](#interacting-with-custom-contracts "Direct link to heading")
