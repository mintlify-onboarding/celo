---
title: "Run Baklava Testnet Validator"
sidebarTitle: "Baklava Validator"
description: "How to get a Validator node running on the Celo Baklava Testnet (Celo's Node Operator Testnet)."
---

## Why run a Baklava Testnet Validator?[](#why-run-a-baklava-testnet-validator "Direct link to heading")
...
<Info>
  If you are transitioning from the Baklava network prior to the June 24 reset,
  you will need to start with a fresh chain database. You can create new nodes
  from fresh machines, as described in this guide, or you may delete your
  chaindata folder, which is named `celo` in the node data directory, and start
  over by running the provided `init` commands for each node described below.
  All on-chain registration steps, the commands completed with `celocli`, will
  need to be run on the new network.
...
### Staking Requirements[](#staking-requirements "Direct link to heading")
...
### Hardware requirements[](#hardware-requirements "Direct link to heading")
...
### Networking requirements[](#networking-requirements "Direct link to heading")
...
### Software requirements[](#software-requirements "Direct link to heading")

#### On each machine[](#on-each-machine "Direct link to heading")
...
#### On your local machine[](#on-your-local-machine "Direct link to heading")
...
### Key Management[](#key-management "Direct link to heading")
...
#### Unlocking[](#unlocking "Direct link to heading")
...
## Validator Node Setup[](#validator-node-setup "Direct link to heading")
...
### Environment Variables[](#environment-variables-1 "Direct link to heading")
...
### Pull the Celo Docker image[](#pull-the-celo-docker-image "Direct link to heading")
...
The `us.gcr.io/celo-org/geth:baklava` image contains the [genesis block](https://github.com/celo-org/celo-monorepo/tree/master/packages/celotool/genesis%5Fbaklava.json) in addition to the Celo Blockchain binary.

#### Account Creation[](#account-creation "Direct link to heading")

<Info>Please complete this section if you are new to validating on Celo.</Info>

##### Account and Signer keys[](#account-and-signer-keys "Direct link to heading")
...
##### Generating Validator and Validator Group Keys[](#generating-validator-and-validator-group-keys "Direct link to heading")

First, you'll need to generate account keys for your Validator and Validator Group.

<Warning>
  These keys will control your locked CELO, and thus should be handled with
  care. Store and back these keys up in a secure manner, as there will be no way
  to recover them if lost or stolen.
</Warning>
...
### Start your Accounts node[](#start-your-accounts-node "Direct link to heading")
...
<Warning>
  **Security**: The command line above includes the parameter `--http.addr
  0.0.0.0` which makes the Celo Blockchain software listen for incoming RPC
  requests on all network adaptors. Exercise extreme caution in doing this when
  running outside Docker, as it means that any unlocked accounts and their funds
  may be accessed from other machines on the Internet. In the context of running
  a Docker container on your local machine, this together with the `docker -p
  127.0.0.1:localport:containerport` flags allows you to make RPC calls from
  outside the container, i.e from your local host, but not from outside your
  machine. Read more about [Docker
  Networking](https://docs.docker.com/network/network-tutorial-standalone/#use-user-defined-bridge-networks)
  here.
</Warning>

### Deploy a Validator[](#deploy-a-validator "Direct link to heading")
...
#### Proof-of-Possession[](#proof-of-possession "Direct link to heading")

<Info>
  Please complete this step if you are running a validator on Celo for the first
  time.
</Info>
...
### Deploy a proxy[](#deploy-a-proxy "Direct link to heading")
...
<Info>
  You can detach from the running container by pressing `ctrl+p ctrl+q`, or
  start it with `-d` instead of `-it` to start detached. Access the logs for a
  container in the background with the `docker logs` command.
</Info>
...
### Get your Proxy's connection info[](#get-your-proxys-connection-info "Direct link to heading")
...
### Connect the Validator to the Proxy[](#connect-the-validator-to-the-proxy "Direct link to heading")
...
<Info>
  You can run multiple proxies by deploying additional proxies per the
  instructions in the [Deploy a
  proxy](/network/baklava/run-validator#deploy-a-proxy) section. Then add all of
  the proxies' enodes as a comma seperated list using the
  `--proxy.proxyenodeurlpairs` option. E.g. if there are two proxies, that
  option's usage would look like
  `--proxy.proxyenodeurlpairs=enode://$PROXY_ENODE_1@$PROXY_INTERNAL_IP_1:30503\;enode://$PROXY_ENODE_1@$PROXY_EXTERNAL_IP_1:30303,enode://$PROXY_ENODE_2@$PROXY_INTERNAL_IP_2:30503\;enode://$PROXY_ENODE_2@$PROXY_EXTERNAL_IP_2:30303`
</Info>

## Registering as a Validator[](#registering-as-a-validator "Direct link to heading")

### Register the Accounts[](#register-the-accounts "Direct link to heading")
...
### Lock up CELO[](#lock-up-celo "Direct link to heading")
...
### Run for election[](#run-for-election "Direct link to heading")
...
<Info>
  **Roadmap**: Different parameters will govern elections in a Celo production
  network. Epochs are likely to be daily, rather than hourly. Running a
  Validator will also include setting up proxy nodes to protect against DDoS
  attacks, and using hardware wallets to secure the key used to sign blocks. We
  plan to update these instructions with more details soon.
</Info>
...
## Deployment Tips[](#deployment-tips "Direct link to heading")

### Running the Docker containers in the background[](#running-the-docker-containers-in-the-background "Direct link to heading")
...
### Stopping containers[](#stopping-containers "Direct link to heading")
