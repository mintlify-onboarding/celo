...
## Why Rotate Keys?[](#why-rotate-keys "Direct link to heading")
...
### Validator Signer Rotation[](#validator-signer-rotation "Direct link to heading")
...
<Warning>
  Before proceeding to step 2 ensure there is sufficient time until the end of
  the epoch to complete key rotation.
</Warning>
...
<Warning>
  Please note that the BLS key will change along with the validator signer ECDSA
  key on the node. If the new BLS key is not authorized, then the validator will
  be unable to process aggregated signatures during consensus, **resulting in
  downtime**. For more details, please read [the BLS key section of the Celo
  account role
  descriptions](/validator/key-management/detailed#authorized-validator-bls-signers).
</Warning>
...
<Warning>
  The newly authorized keys will only take effect in the next epoch, so the
  instance operating with the old key must remain running until the end of the
  current epoch to avoid downtime.
</Warning>
