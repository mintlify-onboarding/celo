...
1. Authorize the new Validator signer key with the Locked Gold Account to overwrite the old Validator signer key.
...
1. If `VALIDATOR_ACCOUNT_ADDRESS` is a `ReleaseGold` contract:
...
1. Shut down the validator instance with the now obsolete signer key.