...
sidebarTitle: "Upgrade a Celo Node"
...
## When an upgrade is required[](#when-an-upgrade-is-required "Direct link to heading")
...
## Upgrading a non-validating node[](#upgrading-a-non-validating-node "Direct link to heading")
...
### Pull the latest Docker image[](#pull-the-latest-docker-image "Direct link to heading")
...
### Stop and remove the existing node[](#stop-and-remove-the-existing-node "Direct link to heading")
...
### Start the new node[](#start-the-new-node "Direct link to heading")
...
## Upgrading a Validating Node[](#upgrading-a-validating-node "Direct link to heading")
...
### Hotswapping Validator Nodes[](#hotswapping-validator-nodes "Direct link to heading")

<Info>
  Hotswap is being introduced in version 1.2.0\. When upgrading nodes that are
  not yet on 1.2.0 refer to the guide to perform a key rotation.
</Info>
...
<Warning>
  Note that the replica node **must** use the same set of proxies as the primary
  node. If it does not it will not be able to switchover without downtime due to
  needing to the complete the announce protocol from scratch. Replicas behind
  the same set of proxies as the primary node will be able to switchover without
  downtime.
</Warning>

#### RPC Methods[](#rpc-methods "Direct link to heading")
...
#### Geth Flags[](#geth-flags "Direct link to heading")
...
<Warning>
  If reconfiguring a node to be a replica or reusing a data directory, make sure
  that the node was previously configured as replica or that the `replicastate`
  folder is removed. If there is an existing `replicastate` folder from a node
  that was not configured as a replica the node will attempt to start
  validating.
</Warning>

#### Steps to upgrade[](#steps-to-upgrade "Direct link to heading")
...
### Upgrading Proxy Nodes[](#upgrading-proxy-nodes "Direct link to heading")

<Warning>
  Release 1.2.0 is backwards incompatible in the Validator and Proxy connection.
  Validators and proxies must be upgraded to 1.2.0 at the same time.
</Warning>
